...
module.exports = function(app, router) { 
    ...
    app.get('/api/category/:media/:category', async (req, res, next) => {
        ...
        const resultJSON = await result.json()
        res.json(processor(resultJSON['results'], req.params.media));
    });
    ...
    */
     ...
     app.get('/api/cast/:media/:id', async (req, res, next) => {
        ...
        const resultJSON = await result.json()
        res.json(detailsProcessorCast(resultJSON['cast'].slice(0,8)));
    });
     ...
     */
     app.get('/api/reviews/:media/:id', async (req, res, next) => {
        ...
        const resultJSON = await result.json()
        res.json(reviewsProcessor(resultJSON['results'].slice(0, 10)));
    });
    ...
    **/
    ...
    app.get('/api/multi/:search', async (req, res, next) => {
        ...
        process_results = process_results['results'].slice(0,7);
    ...
    function processor (process_results, media = 'movie') {
        ...
        process_results.forEach(element => { 
            object.push({
                ...
                poster_path: element['poster_path'] !== null ? 'https://image.tmdb.org/t/p/w500' + element['poster_path'] : './assets/movie_placeholder.png'
            });
        });
    ...
    function detailsProcessorMedia(result, media='movie') {
        return {
            ...
            poster_path: result['poster_path'] !== null ? `https://image.tmdb.org/t/p/w500${result['poster_path']}` : './assets/ReviewsPlaceholderImage.jpg'
        };
    ...
    function reviewsProcessor(results) {
        ...
        results.forEach(element => {
            object.push({ 
                ...
                created_at: new Date(Date.parse(element['created_at'])).toString().slice(0,24),
                url: element['url'],
                rating: element['author_details']['rating'],
    ...
    function detailsProcessorPeople(results) {
        return {
            ...
            profile_path: results['profile_path'] !== null ? 'https://image.tmdb.org/t/p/w500' + results['profile_path'] : './assets/ReviewsPlaceholderImage.jpg'
        };
